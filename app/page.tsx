		...
		<Article image={image as FileField} href={href} noMargin={true}>
			<div className={s.container}>
				{links?.map((link, idx) => {
					...
					const type = t === 'ExternalLinkRecord' ? 'news' : datePeriod(link.startDate, link.endDate);
					const path = t === 'HappeningRecord' ? `/happenings` : t === 'ExhibitionRecord' ? `/exhibitions` : null;
					...
					return (
						...
						>
							{idx > 0 && image?.responsiveImage && (
								<Image className={s.linkImage} data={image.responsiveImage} intersectionMargin='0px 0px 100% 0px' />
							)}
