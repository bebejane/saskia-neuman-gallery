...
export default function Menu(props) {
	...
	useEffect(() => {
		...
		if (scrollY > threshold && darkTheme && imageTheme === "dark") setDarkTheme(false);
		else if (scrollY < threshold) setDarkTheme(imageTheme === "dark");
		...
		more: m.all
			?.filter(
				({ startDate, endDate, id }) =>
					datePeriod(startDate, endDate) === "past" && m.past?.id !== id
			)
			.map((item, idx) => (
				<Link
					key={`more-${idx}`}
	...
	const showSeparator =
		subMenu && showMenu && menu.filter(({ sub, type }) => type === subMenu?.type).length;
	const navbarStyles = cn(
		styles.navbar,
		!showMenu && !showMobileMenu && styles.hide,
		darkTheme && !(subMenu || showMobileMenu) && styles.dark
	);
					...
					<ul>
						{menu.slice(1).map((m, idx) => (
							<li
								...
								onTouchEnd={() =>
									setSubMenuMobile(subMenuMobile && subMenuMobile.label === m.label ? undefined : m)
								}
							>
								<span>
									{m.label}{" "}
									<span
										className={cn(
											styles.arrow,
											(subMenu?.type === m.type || subMenuMobile?.type === m.type) && styles.open,
											hoverSubMenu?.type !== m.type && styles.hide
										)}
									>
										›
									</span>
								...
								{showMobileMenu && m.type === subMenuMobile?.type && (
									<ul
										...
										key={`mobile-list-${idx}`}
										id={`sub-${m.type}`}
										className={cn(subMenuMobile?.type === m.type && styles.open)}
									>
										{m.sub.length > 0 ? m.sub : "To be announced..."}
										{m.more && m.more?.length > 0 && (
											<li className={styles.more}>
												<div
													onClick={() => setShowMore({ ...showMore, [m.type]: !showMore[m.type] })}
												>
													<h3>
														More{" "}
														<div className={cn(styles.arrow, showMore[m.type] && styles.opened)}>
															›
														</div>
													</h3>
												</div>
										...
										{more && more?.length > 0 && (
											<li className={styles.more}>
												<div onClick={() => setShowMore({ ...showMore, [type]: !showMore[type] })}>
													<h3>
														More{" "}
														<div className={cn(styles.arrow, showMore[type] && styles.opened)}>
															›
														</div>
													</h3>
												</div>
