import s from './Happenings.module.scss';
...
export const getStaticProps = withGlobalProps({ model: 'happening' }, async ({ props, context, revalidate }) => {
	const { happening } = await apiQuery(GetHappening, { slug: context.params.slug[0] }, context.preview);

	if (!happening) return { notFound: true, revalidate };
